...
export default function MechanismView({ state, r, l, omega }) {
  ...
  useEffect(() => {
    ...
    // Velocity vectors — tanpa label
    const maxV = Math.max(vBmag, Math.abs(state.vC), 0.01)
    const vSc  = 50 / maxV
    drawArrow(ctx, B.x, B.y, B.x + state.vB.x*vSc, B.y - state.vB.y*vSc, "#a3e635")
    drawArrow(ctx, C.x, C.y - sh/2 - 6, C.x + state.vC*vSc, C.y - sh/2 - 6, "#f0abfc")

    // Force vectors — tanpa label
    const maxF = Math.max(Math.abs(state.Fslider), state.Frod, state.Fpin, 0.01)
    const fSc  = 40 / maxF
    drawArrow(ctx, C.x, C.y+sh/2+6, C.x + state.Fslider*fSc, C.y+sh/2+6, "#ef4444")
    drawArrow(ctx, mid.x, mid.y, mid.x + state.FrodX*fSc, mid.y - state.FrodY*fSc, "#22c55e")
    drawArrow(ctx, B.x, B.y, B.x + state.FpinX*fSc, B.y - state.FpinY*fSc, "#fb923c")

    // Info pojok bawah
    ctx.font = "10px monospace"
    ctx.fillStyle = "rgba(255,255,255,0.25)"; ctx.fillText(`r=${r}  l=${l}  λ=${(r/l).toFixed(2)}`, margin, H-7)
